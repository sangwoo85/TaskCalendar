...
업무 시작일과 종료일이 같은 경우 화면에는 날짜를 한 번만 표시한다.
...
시작일과 종료일이 다른 경우에는 기존처럼 `startDate ~ endDate` 형식으로 표시한다.

```text
2026-06-12 ~ 2026-06-15
```

이 규칙은 주간 timeline, 주간 cardSection, 월간 날짜 cell 업무 item, 월간 progress bar title, 월간 `... N` modal, demo callback alert에 동일하게 적용한다. 날짜 계산, clipping, overlap 규칙은 변경하지 않는다.
